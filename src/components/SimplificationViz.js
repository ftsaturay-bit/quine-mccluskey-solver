        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-800/50 border-b border-white/10">
              <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-black text-slate-400">
                Group
              </th>
              <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-black text-slate-400 text-center">
                Binary
              </th>
              <th className="px-6 py-4 text-[10px] uppercase tracking-widest font-black text-slate-400">
                Minterms Covered
          ...
          {groups.map((group, gIdx) => (
            <tbody
              key={gIdx}
              className="group/row-group border-b border-white/5 last:border-0"
            >
              {group.terms.map((term, tIdx) => (
                ...
                >
                  {tIdx === 0 && (
                    ...
                    >
                      <div className="flex flex-col items-center">
                        <span className="text-2xl font-black text-white/20 group-hover/row-group:text-orange-500/40 transition-colors">
                          {group.ones}
                        </span>
                        <span className="text-[8px] font-bold uppercase tracking-tighter text-slate-600 group-hover/row-group:text-orange-500/40 transition-colors">
                          Ones
                        </span>
                      </div>
                    </td>
                  ...
                  <td className="px-6 py-4 border-b border-white/5 group-last:border-0">
                    <div className="flex flex-wrap gap-1">
                      {term.covers.map((c, i) => (
                        <span
                          key={i}
                          className="px-2 py-0.5 rounded-md bg-slate-800 text-[10px] font-mono text-slate-400 border border-white/5 group-hover/row-group:text-orange-400 group-hover/row-group:border-orange-500/20 transition-all"
                        >
                          {c}
                        </span>
                      ))}
                  ...
                  <td className="px-6 py-4 border-b border-white/5 group-last:border-0 text-center">
                    {!term.isCombined && (
                      <div className="flex justify-center">
                        <span className="w-6 h-6 flex items-center justify-center rounded-full bg-orange-500/20 text-orange-400 border border-orange-500/40 text-[10px] font-black shadow-[0_0_10px_rgba(249,115,22,0.2)] animate-pulse">
                          ✓
                        </span>
                      </div>
                    )}
